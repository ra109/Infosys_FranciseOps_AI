...
5. [Additional Platform Features](#additional-platform-features)
6. [Authentication, OTP & Security](#authentication-otp--security)
7. [Admin Dashboard](#admin-dashboard)
8. [Screenshots](#screenshots)
9. [Installation & Run Instructions](#installation--run-instructions)
10. [requirements.txt](#requirementstxt)
11. [Demo Video](#demo-video)
12. [Known Limitations & Future Scope](#known-limitations--future-scope)
...
Beyond the 9 core agents, the sidebar includes the following supporting modules:

- **Notifications** — real-time in-app alerts (e.g. stock-out risk, audit flags, high attrition risk) surfaced across the platform as they're generated by the agents.
- **Knowledge Graph** — a relationship graph view connecting outlets, staff, inventory, and audit entities, used to trace how a metric or issue in one area links to another.
- **Digital Twin** — a live simulated model of outlet operations used to test "what-if" scenarios (e.g. staffing changes, demand spikes) before applying them in the real world.
- **Anomaly Scanner** — a standalone anomaly detection sweep across outlet, inventory, and audit data (built on Isolation Forest) that flags unusual patterns outside the normal per-agent risk scoring.
- **Data Feed Center** — the ingestion point for uploading or syncing new outlet, staff, inventory, and feedback data, which feeds `seed_data.py`/the database layer.
- **Admin Dashboard** — see [Admin Dashboard](#admin-dashboard) below.
- **Sign Out** — standard session termination, clears the JWT session.
